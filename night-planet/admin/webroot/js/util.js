...
var fileConvert = function (canvasArea, imgList) {
    ...
    $(imgList).each(function (index, element) {
        ...
        if (processingCapacity <= originalBlob["size"]) {
            ...
            var processingBinary = canvasImage.toDataURL(
                "image/jpeg",
                capacityRatio
            ); //画質落としてバイナリ化
...
var fullcalendarSetting = function () {
    ...
    $("#calendar").fullCalendar({
        ...
        dayClick: function (date, jsEvent, view) {
            ...
            var isEvent = false;
            $("#calendar").fullCalendar("clientEvents", function (
                event,
                result
            ) {
                if (
                    event.start.format("Y-MM-DD") == date.format() &&
            ...
            if (!isEvent) {
                ...
                $($form)
                    .find("input[name='end']")
                    .val(date.format("YYYY-MM-DD")); // TODO:１日スパンで登録を行うのでendにも同じ日付を入れておく
        ...
        eventClick: function (calEvent, jsEvent, view) {
            ...
            $($form)
                .find("input[name='start']")
                .val(calEvent["start"].format("YYYY-MM-DD"));
            $($form)
                .find("input[name='end']")
                .val(calEvent["start"].format("YYYY-MM-DD")); // TODO:１日スパンで登録を行うのでendにも同じ日付を入れておく
...
(function (r) {
    function l(a) {
        return b(a, c, t, function (a) {
            return u[a];
        });
    }
    function m(a) {
        return b(a, f, v, function (a) {
            return w[a];
        });
    }
    function b(a, b, d, e) {
        return a && d.test(a) ? a.replace(b, e) : a;
    }
    function d(a) {
        if (null == a) return "";
    ...
    var u = {
        ...
        v = new RegExp(f.source),
        e = /<(?:.|\n)*?>/gm,
        n = new RegExp(e.source),
        g = /([^>\r\n]?)(\r\n|\n\r|\r|\n)/g,
        p = new RegExp(g.source),
        h = /<br\s*\/?>/gm,
        q = new RegExp(h.source);
    r.fn.textWithLF = function (a) {
        var c = typeof a;
        return "undefined" == c
            ? m(b(b(this.html(), h, q, "\n"), e, n, ""))
            : this.html(
