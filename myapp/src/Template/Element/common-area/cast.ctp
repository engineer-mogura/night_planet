      ...
      <div class="cast-header">
        ...
        <div class="cast-header-icon" style="height:70px;">
          ...
          <div class="col s4 m4 count-box">
            <span class="header-diary icon-vertical-align"><?=count($gallery)?></span>
            <span class="header-gallery icon-vertical-align"><?=count($cast->diarys)?></span>
          </div>
          <div class="col s3 m3 today-work-box">
            <span class="header-working icon-vertical-align center-align <?=$isWorkDay > 0 ? " bound":""?>"><?=$isWorkDay > 0 ? "本日出勤予定":"本日出勤未定"?></span>
          </div>
        </div>
      </div>
      <!-- ヘッダー END -->
        ...
        <div class="col s12 center-align">
          <?php if($add_show_flg): ?>
            <a href="<?=DS.$shopInfo['area']['path'].DS.PATH_ROOT['GALLERY'].DS.$cast->id."?area=".$cast->shop->area."&genre=".$cast->shop->genre.
                    ...
                    class="right waves-effect waves-light btn-large"><i class="material-icons right">chevron_right</i><?=COMMON_LB['052']?>
            </a>
          <?php endif;?>
        </div>
      </div>
            ...
            <?php foreach($other_casts as $other_cast): ?>
              <div class="cast-icon-list center-align col s3 m3 l3<?=isset($cast->new_cast) ? ' bound':''?>">
                <a href="<?=DS.$cast->shop['area'].DS.PATH_ROOT['CAST'].DS.$other_cast['id']."?genre=".$cast->shop['genre']."&name=".$cast->shop['name']."&shop=".$cast->shop['id']."&nickname=".$other_cast['nickname']?>">
                  <img src="<?=isset($other_cast->icon) ? $shopInfo['shop_path'].DS.PATH_ROOT['CAST'].DS.$other_cast->dir.DS.PATH_ROOT['PROFILE'].DS.$other_cast->icon : PATH_ROOT['NO_IMAGE02'] ?>" alt="" class="circle" width="100%" height="80">
                </a>
