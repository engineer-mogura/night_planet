    ...
    <div class="row">
      ...
      <div class="col s12 m6 l6">
          ...
          <tr>
            ...
            <td class="show-job-name"><?php if(!$shop->name == '') {
              echo ($shop->name);
            } else {echo ('登録されていません。');}?>
            </td>
          </tr>
          ...
          <th align="center">時間</th>
            <td><?php if((!$job->work_from_time == '')
                      && (!$job->work_to_time == '')) {
                        $workTime = $this->Time->format($job->work_from_time, 'HH:mm')
                        ."～".$this->Time->format($job->work_to_time, 'HH:mm');
                        if (!$job->work_time_hosoku == '') {
          ...
          <td><?php if((!$job->from_age == '')
                      && (!$job->to_age == '')) {
                        $qualification = $job->from_age."歳～".$job->to_age."歳くらいまで";
                        if (!$job->qualification_hosoku == '') {
          ...
          <th align="center">休日</th>
            <td><?php if(!$job->holiday == '') {
                        $holiday = $job->holiday;
                        if (!$job->holiday_hosoku == '') {
