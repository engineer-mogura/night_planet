...
<article id="SNS_Instagram">
    <header>
        <section>
		<h1>
			<?php $is_link_enable = false; ?>
			<?php ($shopInfo['current_plan'] == SERVICE_PLAN['preminu_s']) ? $is_link_enable = true  : null ?>
			<a href="https://www.instagram.com/" style="<?= $is_link_enable ? 'pointer-events:none; ' : null ?>" target="new" rel="noopener">
            ...
            <div>
					<a href="https://www.instagram.com/<?php echo $ig_data->username; ?>/" style="<?= $is_link_enable ? 'pointer-events:none; ' : null ?>" target="new" rel="noopener">
					<?php echo $ig_data->name."\n"; ?>
					(<?php echo $test; ?>@<?php echo $ig_data->username; ?>)
					</a>
            </div>
        </section>
        <nav>
