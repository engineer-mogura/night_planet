                ...
                <?php if(count($diarys) > 0) { ?>
                    <!-- <ul class="collection card-panel archive-panel">
                        ...
                        <?php !empty($row['image1'])? $imgPath = $userInfo['diary_path'].$row['dir'].DS.$row['image1'] : $imgPath = PATH_ROOT['NO_IMAGE01']; ?>
                            ...
                            <span class="title"><?=$this->Text->excerpt($row->title, 'method', PROPERTY['TITLE_EXCERPT'], PROPERTY['ELLIPSIS']); ?></span><span class="badge"><?= $row['md_created'] ?></span>
                            <p><?= $this->Text->excerpt($row['content'], 'method', PROPERTY['CONTENT_EXCERPT'], PROPERTY['ELLIPSIS']);?></p>
